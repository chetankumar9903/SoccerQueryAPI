...
namespace SoccerQueryAPI.Data
{
    public class SqlValidator
    {
        ...
        public bool IsSelectOnly(string sql)
        {
            ...
            // Only allow statements that start with SELECT (case-insensitive)
        ...
        public bool ContainsOnlyAllowedTablesAndColumns(string sql, out string reason)
        {
            ...
            // quick fail if any suspicious statements are present
            ...
            foreach (var t in tokens)
            {
                ...
                if (t.Length > 0 && char.IsLetter(t[0]) && t.Length <= 10) continue; // lenient: some tokens may be aliases or functions
                                                                                     // If token not recognized, we don't block immediately but note it
            }
        ...
        {
            // If SQL already has LIMIT, do not change; otherwise append LIMIT
